...
namespace Water2D
{
    ...
    public class WaveSimulation
    {
        ...
        public void Collision(Collider2D c, float t)
        {
            ...
            Vector2 rbLinearVelocity = WaterUnityCompatibility.GetRigidbody2DLinearVelocity(rb);
            float fa = settings.splashForceMin.value;
            float fe = settings.splashForceMax.value;
            float ft = Mathf.Lerp(0f, 1f, ((0.5f * rb.mass * rbLinearVelocity.y * rbLinearVelocity.y) - fa) / (fe - fa));
